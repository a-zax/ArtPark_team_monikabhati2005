...
exports.generateAdaptivePathway = generateAdaptivePathway;
...
function buildKnownSkillSet(analysis) {
    return new Set(analysis.candidate_profile
        .filter((item) => {
        ...
        .map((item) => (0, analysis_types_1.normalizeSkillName)(item.skill)));
}
function getCandidateSignal(analysis, skill) {
    return analysis.candidate_profile.find((item) => (0, analysis_types_1.normalizeSkillName)(item.skill) === (0, analysis_types_1.normalizeSkillName)(skill));
}
function isLowConfidenceSkill(analysis, skill) {
...
function isStaleSkill(analysis, skill) {
    const profile = getCandidateSignal(analysis, skill);
    return Boolean(profile?.is_stale || (typeof profile?.decay_score === 'number' && profile.decay_score < STALE_SKILL_THRESHOLD));
}
function getRequiredLevel(analysis, skill) {
    const profile = analysis.required_profile.find((item) => (0, analysis_types_1.normalizeSkillName)(item.skill) === (0, analysis_types_1.normalizeSkillName)(skill));
    return profile?.level ?? 'intermediate';
}
function getRequiredWeight(analysis, skill) {
    return analysis_types_1.SKILL_LEVEL_WEIGHT[getRequiredLevel(analysis, skill)];
}
function scoreCourse(course, skill, analysis) {
    const normalizedSkill = (0, analysis_types_1.normalizeSkillName)(skill);
    const exactMatch = course.skills_covered.some((coveredSkill) => (0, analysis_types_1.normalizeSkillName)(coveredSkill) === normalizedSkill);
    const requiredLevel = analysis_types_1.SKILL_LEVEL_WEIGHT[getRequiredLevel(analysis, skill)];
    const difficultyLevel = analysis_types_1.SKILL_LEVEL_WEIGHT[course.difficulty];
...
function resolveCoursePath(courseId, selectedIds, activeCatalog) {
...
function mapCourses(analysis, activeCatalog) {
    const selectedIds = new Set();
    const matchedMissingSkills = new Set();
    const unmatchedMissingSkills = [];
    for (const skill of analysis.missing_skills) {
        const candidates = activeCatalog
            .filter((course) => course.skills_covered.some((coveredSkill) => (0, analysis_types_1.normalizeSkillName)(coveredSkill) === (0, analysis_types_1.normalizeSkillName)(skill)))
...
function buildReasoning(course, analysis, refreshType) {
    const targetedSkills = course.skills_covered.filter((skill) => analysis.missing_skills.map(analysis_types_1.normalizeSkillName).includes((0, analysis_types_1.normalizeSkillName)(skill)));
    const prerequisiteText = course.prerequisites && course.prerequisites.length > 0
        ? `Prerequisites included: ${course.prerequisites.join(', ')}.`
        : 'No prerequisite modules were required ahead of this step.';
    const refreshText = refreshType === 'partial-confidence'
...
function calculateRoi(analysis, activeCatalog) {
    ...
    for (const course of activeCatalog) {
        ...
        const knownCoverage = covered.filter((skill) => knownSkills.has((0, analysis_types_1.normalizeSkillName)(skill))).length;
        if (knownCoverage === 0)
            continue;
...
function pickTopGap(analysis) {
    return (analysis.required_profile
        .filter((item) => analysis.missing_skills.map(analysis_types_1.normalizeSkillName).includes((0, analysis_types_1.normalizeSkillName)(item.skill)))
        .sort((a, b) => analysis_types_1.SKILL_LEVEL_WEIGHT[b.level] - analysis_types_1.SKILL_LEVEL_WEIGHT[a.level])[0]?.skill ??
        analysis.missing_skills[0] ??
        'role-specific onboarding');
}
function generateAdaptivePathway(analysis, domainType = 'knowledge') {
    ...
    const weightedUnmatchedPenalty = unmatchedMissingSkills.reduce((sum, skill) => sum + getRequiredWeight(analysis, skill) * 10, 0);
    const baseMissingPenalty = analysis.missing_skills.reduce((sum, skill) => sum + getRequiredWeight(analysis, skill) * 6, 0);
    const roleReadinessScore = Math.max(20, 100 - baseMissingPenalty - weightedMatchedPenalty - weightedUnmatchedPenalty);
    const coverageRatio = analysis.missing_skills.length === 0
        ? 1
        : matchedMissingSkills.length / analysis.missing_skills.length;
    const pathway = selectedCourses.map((course) => {
        const skillsTargeted = course.skills_covered.filter((skill) => analysis.missing_skills.map(analysis_types_1.normalizeSkillName).includes((0, analysis_types_1.normalizeSkillName)(skill)));
        ...
        const refreshType = lowConfidenceRefresh ? 'partial-confidence' : staleRefresh ? 'partial-stale' : 'full';
        const partialTitleSuffix = refreshType === 'partial-confidence'
            ? ' (Confidence Refresh)'
            : refreshType === 'partial-stale'
                ? ' (Decay Refresh)'
                : '';
        return {
            ...
            estimated_hours: refreshType === 'partial-confidence'
    ...
    const mentorTitle = mentorRoleBase === 'advanced'
        ? 'Principal Mentor'
    ...
    return {
        ...
        sandbox_project: {
            ...
            description: unmatchedMissingSkills.length > 0
